...
const PricingPlans: React.FC<PricingPlansProps> = ({ data, isYearly }) => (
    <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
        {data.map((plan) => (
            <div
                key={plan.name}
                className={`rounded-lg border border-[#3C3C77] bg-[#0F0F24] p-8 text-start transition duration-300 ease-in-out hover:border-[#ACDA49] hover:shadow-lg ${plan.popular ? 'relative' : ''
                    }`}
            >
                {plan.popular && (
                    <span className="absolute right-2 top-2 rounded-md bg-[#fedc78] px-2 py-1 text-xs font-bold">
                        Popular
                    </span>
                )}
                <h3 className="mb-4 text-2xl font-bold text-[#5772fd] md:text-3xl">
                    {plan.name}
                </h3>
                ...
                <p className="mb-6 text-lg font-bold text-gray-50">
                    {plan.billingInfo}
                </p>
