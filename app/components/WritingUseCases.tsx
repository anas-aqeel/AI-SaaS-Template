...
const WritingUseCases: React.FC<WritingUseCasesProps> = ({ data }) => (
  <section className="bg-[#111022] py-12 font-sans text-white md:py-20">
    <div className="mx-auto max-w-7xl px-4 text-center">
      <h2 className="mb-4 text-xl font-bold text-[#ACDA49] md:text-2xl">Writing Use Cases</h2>
        ...
        {data.map((useCase, index) => (
          <div key={index} className="group relative rounded-lg border border-[#3C3C77] p-8">
            <div className="mb-6 flex items-center justify-start">
              <useCase.icon className={`size-8 text-[#fedc78]`} />
            </div>
            <h4 className="mb-4 text-xl font-bold text-white md:text-lg">{useCase.title}</h4>
            <p className="mb-6 text-sm text-gray-400 md:text-base">{useCase.description}</p>
